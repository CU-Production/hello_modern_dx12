...
HRESULT GPUCompressBC::Initialize(ID3D11Device* pDevice)
{
    ...
    // Modes 11-14
    HRESULT hr = pDevice->CreateComputeShader(cs5::BC6HEncode_TryModeG10CS, sizeof(cs5::BC6HEncode_TryModeG10CS), nullptr, m_BC6H_tryModeG10CS.ReleaseAndGetAddressOf());
    if (FAILED(hr))
        return hr;

    // Modes 1-10
    hr = pDevice->CreateComputeShader(cs5::BC6HEncode_TryModeLE10CS, sizeof(cs5::BC6HEncode_TryModeLE10CS), nullptr, m_BC6H_tryModeLE10CS.ReleaseAndGetAddressOf());
    if (FAILED(hr))
        return hr;

    // Encode
    hr = pDevice->CreateComputeShader(cs5::BC6HEncode_EncodeBlockCS, sizeof(cs5::BC6HEncode_EncodeBlockCS), nullptr, m_BC6H_encodeBlockCS.ReleaseAndGetAddressOf());
    if (FAILED(hr))
        return hr;
    ...
    // Modes 4, 5, 6
    hr = pDevice->CreateComputeShader(cs5::BC7Encode_TryMode456CS, sizeof(cs5::BC7Encode_TryMode456CS), nullptr, m_BC7_tryMode456CS.ReleaseAndGetAddressOf());
    if (FAILED(hr))
        return hr;

    // Modes 1, 3, 7
    hr = pDevice->CreateComputeShader(cs5::BC7Encode_TryMode137CS, sizeof(cs5::BC7Encode_TryMode137CS), nullptr, m_BC7_tryMode137CS.ReleaseAndGetAddressOf());
    if (FAILED(hr))
        return hr;

    // Modes 0, 2
    hr = pDevice->CreateComputeShader(cs5::BC7Encode_TryMode02CS, sizeof(cs5::BC7Encode_TryMode02CS), nullptr, m_BC7_tryMode02CS.ReleaseAndGetAddressOf());
    if (FAILED(hr))
        return hr;

    // Encode
    hr = pDevice->CreateComputeShader(cs5::BC7Encode_EncodeBlockCS, sizeof(cs5::BC7Encode_EncodeBlockCS), nullptr, m_BC7_encodeBlockCS.ReleaseAndGetAddressOf());
    if (FAILED(hr))
        return hr;
