    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                texture_handle.ptr =  bd->FontTexture.hFontSrvGpuDescHandles[(vd->FrameIndex + 1) % bd->numFramesInFlight].ptr; //(UINT64)pcmd->GetTexID();
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    ImGui_ImplDX12_Texture* font_tex = &bd->FontTexture;
    {
        ...
        bd->pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc, font_tex->hFontSrvCpuDescHandles[0]);
        bd->pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc, font_tex->hFontSrvCpuDescHandles[1]);
        SafeRelease(font_tex->pTextureResource);
        font_tex->pTextureResource = pTexture;
    }

    // Store our identifier
    io.Fonts->SetTexID((ImTextureID)font_tex->hFontSrvGpuDescHandles[0].ptr);
}
...
void    ImGui_ImplDX12_InvalidateDeviceObjects()
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    ImGui_ImplDX12_Texture* font_tex = &bd->FontTexture;
    bd->InitInfo.SrvDescriptorFreeFn(&bd->InitInfo, font_tex->hFontSrvCpuDescHandles[0], font_tex->hFontSrvGpuDescHandles[0], font_tex->hFontSrvCpuDescHandles[1], font_tex->hFontSrvGpuDescHandles[1]);
    SafeRelease(font_tex->pTextureResource);
...
bool ImGui_ImplDX12_Init(ImGui_ImplDX12_InitInfo* init_info)
{
    ...
    if (init_info->SrvDescriptorAllocFn == nullptr)
    {
        ...
        IM_ASSERT(init_info->LegacySingleSrvCpuDescriptors[0].ptr != 0 && init_info->LegacySingleSrvGpuDescriptors[0].ptr != 0);
        IM_ASSERT(init_info->LegacySingleSrvCpuDescriptors[1].ptr != 0 && init_info->LegacySingleSrvGpuDescriptors[1].ptr != 0);
        init_info->SrvDescriptorAllocFn = [](ImGui_ImplDX12_InitInfo*, D3D12_CPU_DESCRIPTOR_HANDLE* out_cpu_handle, D3D12_GPU_DESCRIPTOR_HANDLE* out_gpu_handle, D3D12_CPU_DESCRIPTOR_HANDLE* out_cpu_handle2, D3D12_GPU_DESCRIPTOR_HANDLE* out_gpu_handle2)
        {
            ...
            *out_cpu_handle = bd->InitInfo.LegacySingleSrvCpuDescriptors[0];
            *out_gpu_handle = bd->InitInfo.LegacySingleSrvGpuDescriptors[0];
            *out_cpu_handle2 = bd->InitInfo.LegacySingleSrvCpuDescriptors[1];
            *out_gpu_handle2 = bd->InitInfo.LegacySingleSrvGpuDescriptors[1];
    ...
    init_info->SrvDescriptorAllocFn(&bd->InitInfo, &bd->FontTexture.hFontSrvCpuDescHandles[0], &bd->FontTexture.hFontSrvGpuDescHandles[0], &bd->FontTexture.hFontSrvCpuDescHandles[1], &bd->FontTexture.hFontSrvGpuDescHandles[1]);
...
bool ImGui_ImplDX12_Init(ID3D12Device* device, int num_frames_in_flight, DXGI_FORMAT rtv_format, ID3D12DescriptorHeap* srv_descriptor_heap, D3D12_CPU_DESCRIPTOR_HANDLE font_srv_cpu_desc_handle, D3D12_GPU_DESCRIPTOR_HANDLE font_srv_gpu_desc_handle, D3D12_CPU_DESCRIPTOR_HANDLE font_srv_cpu_desc_handle2, D3D12_GPU_DESCRIPTOR_HANDLE font_srv_gpu_desc_handle2)
{
    ...
    init_info.LegacySingleSrvCpuDescriptors[0] = font_srv_cpu_desc_handle;
    init_info.LegacySingleSrvGpuDescriptors[0] = font_srv_gpu_desc_handle;
    init_info.LegacySingleSrvCpuDescriptors[1] = font_srv_cpu_desc_handle2;
    init_info.LegacySingleSrvGpuDescriptors[1] = font_srv_gpu_desc_handle2;
